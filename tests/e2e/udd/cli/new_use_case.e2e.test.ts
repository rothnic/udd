...
const feature = await loadFeature('specs/features/udd/cli/new_use_case.feature');
...
describeFeature(feature, ({ Scenario }) => {
  Scenario('Create a new use case', ({ Given, When, Then, And }) => {
    ...
    Then('a file "specs/use-cases/my_new_use_case.yml" should exist', async () => {
      await fs.access(path.join(rootDir, 'specs/use-cases/my_new_use_case.yml'));
    });

    And('the file content should contain "id: my_new_use_case"', async () => {
      const content = await fs.readFile(path.join(rootDir, 'specs/use-cases/my_new_use_case.yml'), 'utf-8');

...
1. Link tests to features → `specs/test-governance/test-linkage.feature`  
   Associate tests with the features they validate so changes to a feature automatically surface relevant tests

2. Check test status → `specs/test-governance/test-status.feature`  
   See at a glance which tests are passing, failing, outdated, or need review

3. Mark tests clean → `specs/test-governance/test-review.feature`  

...
#   - Scaffolds are idempotent when appropriate and surface helpful error messages when they are not.
  ...
  Scenario: Create feature when parent directory doesn't exist
    Given I am in a UDD project
  ...
  Scenario: Feature creation overwrites existing feature (duplicate)
    Given I am in a UDD project
  ...
  Scenario: Feature name with slashes creates nested directories
    Given I am in a UDD project
    When I run "udd new feature test_domain invalid/name"
    Then the command should exit with code 0
    And the command should create nested directories and a feature file for the name containing slashes

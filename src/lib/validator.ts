...
export async function validateSpecs(): Promise<ValidationResult> {
  ...
  try {
    ...
    if (!frontmatterMatch) {
      ...
      if (!result.success) {
        errors.push(`specs/VISION.md: Invalid frontmatter - ${result.error.message}`);
      }
    }
  } catch {
      ...
      if (!result.success) {
        errors.push(`${file}: Invalid schema - ${result.error.message}`);
         ...
         for (const slug of result.data.scenarios) {
             ...
             const featurePath = path.join(specsDir, 'features', result.data.feature, `${slug}.feature`);
             try {
                 await fs.access(featurePath);
             } catch {
                 errors.push(`${file}: References missing scenario ${slug} in feature ${result.data.feature}`);
             }
         }
      }
    } catch {
  ...
  const scenarioFiles = await glob('specs/features/**/*.feature', { cwd: rootDir });

  ...
  .action(async (options) => {
    try {
      ...
      if (options.json) {
        ...
        status.active_features.forEach(f => console.log(`- ${f}`));
        ...
        for (const [id, feature] of Object.entries(status.features)) {
            ...
            for (const [slug, sStatus] of Object.entries(feature.scenarios)) {
                const color = sStatus.e2e === 'passing' ? chalk.green : 
                              sStatus.e2e === 'failing' ? chalk.red : chalk.yellow;
                console.log(`    ${slug}: ${color(sStatus.e2e)}`);
            }
            console.log('  Requirements:');
            for (const [key, rStatus] of Object.entries(feature.requirements)) {
                const color = rStatus.tests === 'passing' ? chalk.green : 
                              rStatus.tests === 'failing' ? chalk.red : chalk.yellow;

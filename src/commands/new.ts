...
export const newCommand = new Command('new')
  .description('Scaffold new specs');

newCommand.command('use-case')
...
newCommand.command('feature')
  ...
  .action(async (area, feature) => {
    ...
    const content = {
      id: `${area}/${feature}`,
      area: area,
      name: feature.split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
      summary: "TODO: Add summary",
      use_cases: [],
      phase: 1,
      kind: "core"
...
newCommand.command('scenario')
  ...
  .action(async (area, feature, slug) => {
    ...
    const featureName = feature.split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    const scenarioName = slug.split('_').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
